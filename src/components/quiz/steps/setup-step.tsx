...
export function SetupStep() {
  ...
  const { createSession, language } = useQuiz();
  const t = translations;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!userAName.trim() || !userBName.trim()) return;
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4 input">
              <div className="space-y-2">
                <Label htmlFor="userAName">
                  {t.yourNameLabel[language]}
                </Label>
                <Input
                  id="userAName"
                  value={userAName}
              ...
              <div className="space-y-2">
                <Label htmlFor="userBName">
                  {t.partnerNameLabel[language]}
                </Label>
                <Input
                  id="userBName"
                  value={userBName}
            ...
            <CardFooter>
              <Button
                type="submit"
                className="w-full"
                disabled={isLoading || !userAName || !userBName}
              >
                {isLoading && (
